...
public class ZMorton : MonoBehaviour {
/*@T
 ...
 *@c*/
	...
	public static uint zm_part1by2(uint x){
		x &= 0x000003ff;
		x = (x ^ (x << 16)) & 0xff0000ff;
		x = (x ^ (x <<  8)) & 0x0300f00f;
		x = (x ^ (x <<  4)) & 0x030c30c3;
	...
	public static uint zm_compact1by2(uint x){
		x &= 0x09249249;
		x = (x ^ (x >>  2)) & 0x030c30c3;
		x = (x ^ (x >>  4)) & 0x0300f00f;
		x = (x ^ (x >>  8)) & 0xff0000ff;
	...
	public static uint zm_encode(uint x, uint y, uint z){
		return (zm_part1by2(z) << 2) + (zm_part1by2(y) << 1) + zm_part1by2(x);
	}

	public static void zm_decode(uint code, ref uint x, ref uint y, ref uint z){
